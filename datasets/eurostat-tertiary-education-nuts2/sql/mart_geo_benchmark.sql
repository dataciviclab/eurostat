-- mart_geo_benchmark — Tertiary educational attainment by NUTS2 region:
-- EU27-wide benchmark analytics.
--
-- One row per (year, geo, unit, isced11, age, sex). Replaces the old
...
-- unit = 'PC' AND isced11 = 'ED5-8' AND age = 'Y25-64' AND sex = 'T'
...
--
-- NOTE: higher value = better (higher tertiary attainment).
...
SELECT
    ...
    CASE
        WHEN b.unit = 'PC' AND b.isced11 = 'ED5-8' AND b.age = 'Y25-64' AND b.sex = 'T'
        THEN ROUND(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.isced11, b.age, b.sex), 2)
        ELSE NULL
    END AS media_eu_value,
    -- Country average for the same year, NUTS level, unit and dims
    CASE
        WHEN b.unit = 'PC' AND b.isced11 = 'ED5-8' AND b.age = 'Y25-64' AND b.sex = 'T'
        THEN ROUND(AVG(b.value) OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit, b.isced11, b.age, b.sex), 2)
        ELSE NULL
    END AS media_paese_value,
    -- Percentile within EU27 (same year, nuts_level, unit, dims); NULL outside
    CASE
        WHEN b.unit = 'PC' AND b.isced11 = 'ED5-8' AND b.age = 'Y25-64' AND b.sex = 'T' AND b.is_eu
        THEN ROUND(
            PERCENT_RANK() OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.isced11, b.age, b.sex, b.is_eu ORDER BY b.value), 4)
        ELSE NULL
    END AS percentile_eu,
    -- National rank (1 = highest attainment in the country, same year/level/unit/dims)
    CASE
        WHEN b.unit = 'PC' AND b.isced11 = 'ED5-8' AND b.age = 'Y25-64' AND b.sex = 'T'
        THEN RANK() OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit, b.isced11, b.age, b.sex ORDER BY b.value DESC)
        ELSE NULL
    ...
    CASE
        WHEN b.unit = 'PC' AND b.isced11 = 'ED5-8' AND b.age = 'Y25-64' AND b.sex = 'T'
        THEN ROUND(
            (b.value - AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.isced11, b.age, b.sex))
            / NULLIF(ABS(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.isced11, b.age, b.sex)), 0) * 100, 1)
        ELSE NULL
    END AS distanza_media_eu_pct,
    b.flag,
    b.flag_desc_en
FROM base b
ORDER BY b.year DESC, b.nuts_level, b.country, b.geo, b.unit, b.isced11, b.age, b.sex
