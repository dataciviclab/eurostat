-- clean.sql: TOUR_OCC_NIN2 — nights spent at tourist accommodation by NUTS3
-- region, enriched.
--
-- Extra dimensions vs the base pattern: c_resid (residency: total/foreign/
-- domestic) and nace_r2 (accommodation type), enriched with labels from
-- the support codelists.
--
...
--
-- NOTE: NR = nights spent (thousands). Slice benchmark = unit NR +
-- c_resid TOTAL + nace_r2 I551-I553 (all accommodation, all residency).
...
SELECT
    ...
    c.label_en AS c_resid_label_en,
...
LEFT JOIN read_parquet('{support.c_resid.path}') c ON r.c_resid = c.code
