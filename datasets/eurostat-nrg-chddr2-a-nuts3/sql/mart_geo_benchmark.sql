-- mart_geo_benchmark — Heating degree days by NUTS3 region: EU27-wide
-- benchmark
-- analytics.
--
-- One row per (year, geo, unit, indic_nrg). Replaces the old pass-through
...
-- unit = 'NR' AND indic_nrg = 'HDD' (old-age dependency ratio — the
...
SELECT
    ...
    CASE
        ...
        THEN ROUND(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.indic_nrg), 2)
        ELSE NULL
    END AS media_eu_value,
    -- Country average for the same year, NUTS level, unit and indicator
    CASE
        WHEN b.unit = 'NR' AND b.indic_nrg = 'HDD'
        THEN ROUND(AVG(b.value) OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit, b.indic_nrg), 2)
        ELSE NULL
    ...
    CASE
        ...
        THEN ROUND(
            PERCENT_RANK() OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.indic_nrg, b.is_eu ORDER BY b.value), 4)
        ELSE NULL
    END AS percentile_eu,
    -- National rank (1 = highest fertility in the country, same year/level/unit/indicator)
    CASE
        WHEN b.unit = 'NR' AND b.indic_nrg = 'HDD'
        THEN RANK() OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit, b.indic_nrg ORDER BY b.value DESC)
        ELSE NULL
    ...
    CASE
        WHEN b.unit = 'NR' AND b.indic_nrg = 'HDD'
        THEN ROUND(
            (b.value - AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.indic_nrg))
            / NULLIF(ABS(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.indic_nrg)), 0) * 100, 1)
        ELSE NULL
    END AS distanza_media_eu_pct,
    b.flag,
    b.flag_desc_en
FROM base b
ORDER BY b.year DESC, b.nuts_level, b.country, b.geo, b.unit, b.indic_nrg
