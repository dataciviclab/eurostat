...
SELECT
    ...
    CASE
        WHEN b.indic_sb = 'V11920' AND b.nace_r2 = 'B-S_X_K642'
        THEN ROUND(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.indic_sb, b.nace_r2), 1)
        ELSE NULL
    END AS media_eu_value,
    -- Country average for the same year, NUTS level and dims
    CASE
        WHEN b.indic_sb = 'V11920' AND b.nace_r2 = 'B-S_X_K642'
        THEN ROUND(AVG(b.value) OVER (PARTITION BY b.year, b.country, b.nuts_level, b.indic_sb, b.nace_r2), 1)
        ELSE NULL
    END AS media_paese_value,
    -- Percentile within EU27 (same year, nuts_level, dims); NULL outside
    CASE
        WHEN b.indic_sb = 'V11920' AND b.nace_r2 = 'B-S_X_K642' AND b.is_eu
        THEN ROUND(
            PERCENT_RANK() OVER (PARTITION BY b.year, b.nuts_level, b.indic_sb, b.nace_r2, b.is_eu ORDER BY b.value), 4)
        ELSE NULL
    END AS percentile_eu,
    -- National rank (1 = most births in the country, same year/level/dims)
    CASE
        WHEN b.indic_sb = 'V11920' AND b.nace_r2 = 'B-S_X_K642'
        THEN RANK() OVER (PARTITION BY b.year, b.country, b.nuts_level, b.indic_sb, b.nace_r2 ORDER BY b.value DESC)
        ELSE NULL
    ...
    CASE
        WHEN b.indic_sb = 'V11920' AND b.nace_r2 = 'B-S_X_K642'
        THEN ROUND(
            (b.value - AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.indic_sb, b.nace_r2))
            / NULLIF(ABS(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.indic_sb, b.nace_r2)), 0) * 100, 1)
        ELSE NULL
    END AS distanza_media_eu_pct,
    b.flag,
    b.flag_desc_en
FROM base b
ORDER BY b.year DESC, b.nuts_level, b.country, b.geo, b.indic_sb, b.nace_r2
