-- mart_geo_benchmark — Population density by NUTS3 region: EU27-wide
-- benchmark analytics.
--
...
--
-- Benchmark columns are computed ONLY for unit = 'PER_KM2' (inhabitants
...
SELECT
    ...
    b.unit_label_en,
    b.value,
    -- EU27 average for the same year, NUTS level and unit (PER_KM2 only)
    CASE
        WHEN b.unit = 'PER_KM2' THEN ROUND(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit), 1)
        ELSE NULL
    END AS media_eu_value,
    -- Country average for the same year, NUTS level and unit
    CASE
        WHEN b.unit = 'PER_KM2' THEN ROUND(AVG(b.value) OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit), 1)
        ELSE NULL
    END AS media_paese_value,
    -- Percentile within EU27 (same year, nuts_level, unit); NULL outside EU27
    CASE
        WHEN b.unit = 'PER_KM2' AND b.is_eu THEN ROUND(
            PERCENT_RANK() OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.is_eu ORDER BY b.value), 4)
        ELSE NULL
    END AS percentile_eu,
    -- National rank (1 = highest density in the country, same year/level/unit)
    CASE
        WHEN b.unit = 'PER_KM2' THEN RANK() OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit ORDER BY b.value DESC)
        ELSE NULL
    END AS rank_nazionale,
    -- % distance from the EU27 average (same year, nuts_level, unit)
    CASE
        WHEN b.unit = 'PER_KM2' THEN ROUND(
