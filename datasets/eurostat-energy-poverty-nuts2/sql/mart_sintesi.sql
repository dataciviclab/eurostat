-- mart_sintesi — Energy poverty by country: national aggregates and ranking.
--
-- One row per (country, year). Built from NUTS2-level data of each country:
--   • energy poverty share (country-level PC value, %)
...
country_values AS (
    SELECT
        ...
        MAX(CASE WHEN unit = 'PC' THEN value END) AS energy_poverty_pct
    FROM clean_input
...
SELECT
    cv.year,
    cv.country,
    cv.energy_poverty_pct,
    -- Cross-country rank (EU27 only) by poverty share (1 = highest poverty, same year)
    CASE WHEN cv.is_eu THEN RANK() OVER (PARTITION BY cv.year, cv.is_eu ORDER BY cv.energy_poverty_pct DESC) ELSE NULL END AS rank_procapite_eu,
    -- % distance from the EU27 country average
    ROUND(
        (cv.energy_poverty_pct - AVG(cv.energy_poverty_pct) FILTER (WHERE cv.is_eu) OVER (PARTITION BY cv.year))
        / NULLIF(ABS(AVG(cv.energy_poverty_pct) FILTER (WHERE cv.is_eu) OVER (PARTITION BY cv.year)), 0) * 100, 1
