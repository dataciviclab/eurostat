-- mart_geo_benchmark — Births by age group by NUTS2 region:
-- EU27-wide benchmark analytics.
--
-- One row per (year, geo, unit, age, age). Replaces the old
...
SELECT
    ...
    CASE
        WHEN b.unit = 'NR' AND b.age = 'TOTAL'
        THEN ROUND(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.age), 2)
        ELSE NULL
    END AS media_eu_value,
    -- Country average for the same year, NUTS level, unit and dims
    CASE
        WHEN b.unit = 'NR' AND b.age = 'TOTAL'
        THEN ROUND(AVG(b.value) OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit, b.age), 2)
        ELSE NULL
    END AS media_paese_value,
    -- Percentile within EU27 (same year, nuts_level, unit, dims); NULL outside
    CASE
        WHEN b.unit = 'NR' AND b.age = 'TOTAL'  AND b.is_eu
        THEN ROUND(
            PERCENT_RANK() OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.age, b.is_eu ORDER BY b.value), 4)
        ELSE NULL
    END AS percentile_eu,
    -- National rank (1 = highest attainment in the country, same year/level/unit/dims)
    CASE
        WHEN b.unit = 'NR' AND b.age = 'TOTAL'
        THEN RANK() OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit, b.age ORDER BY b.value DESC)
        ELSE NULL
    ...
    CASE
        WHEN b.unit = 'NR' AND b.age = 'TOTAL'
        THEN ROUND(
            (b.value - AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.age))
            / NULLIF(ABS(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.age)), 0) * 100, 1)
        ELSE NULL
    END AS distanza_media_eu_pct,
    b.flag,
    b.flag_desc_en
FROM base b
ORDER BY b.year DESC, b.nuts_level, b.country, b.geo, b.unit, b.age
