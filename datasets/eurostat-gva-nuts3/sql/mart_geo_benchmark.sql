-- mart_geo_benchmark — Gross value added by NUTS3 region: EU27-wide
-- benchmark
-- analytics.
--
...
-- unit = 'CP_MEUR' AND nace_r2 = 'TOTAL' (old-age dependency ratio — the
...
SELECT
    ...
    CASE
        WHEN b.unit = 'CP_MEUR' AND b.nace_r2 = 'TOTAL'
        THEN ROUND(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.nace_r2), 2)
        ELSE NULL
    END AS media_eu_value,
    -- Country average for the same year, NUTS level, unit and indicator
    CASE
        WHEN b.unit = 'CP_MEUR' AND b.nace_r2 = 'TOTAL'
        THEN ROUND(AVG(b.value) OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit, b.nace_r2), 2)
        ELSE NULL
    END AS media_paese_value,
    -- Percentile within EU27 (same year, nuts_level, unit, indicator); NULL outside
    CASE
        WHEN b.unit = 'CP_MEUR' AND b.nace_r2 = 'TOTAL' AND b.is_eu
        THEN ROUND(
            PERCENT_RANK() OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.nace_r2, b.is_eu ORDER BY b.value), 4)
        ELSE NULL
    END AS percentile_eu,
    -- National rank (1 = highest fertility in the country, same year/level/unit/indicator)
    CASE
        WHEN b.unit = 'CP_MEUR' AND b.nace_r2 = 'TOTAL'
        THEN RANK() OVER (PARTITION BY b.year, b.country, b.nuts_level, b.unit, b.nace_r2 ORDER BY b.value DESC)
        ELSE NULL
    ...
    CASE
        WHEN b.unit = 'CP_MEUR' AND b.nace_r2 = 'TOTAL'
        THEN ROUND(
            (b.value - AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.nace_r2))
            / NULLIF(ABS(AVG(b.value) FILTER (WHERE b.is_eu) OVER (PARTITION BY b.year, b.nuts_level, b.unit, b.nace_r2)), 0) * 100, 1)
        ELSE NULL
