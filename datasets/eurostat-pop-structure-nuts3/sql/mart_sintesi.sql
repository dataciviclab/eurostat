...
-- TOTFERRT):
--   • old-age dependency ratio (country-level value, %)
...
country_values AS (
    SELECT
        ...
        MAX(CASE WHEN unit = 'PC' AND indic_de = 'OLDDEP2' THEN value END) AS dipendenza_anziani_pct
    FROM clean_input
...
SELECT
    cv.year,
    cv.country,
    cv.dipendenza_anziani_pct,
    -- Cross-country rank (EU27 only, 1 = highest fertility)
    CASE WHEN cv.is_eu THEN RANK() OVER (PARTITION BY cv.year, cv.is_eu ORDER BY cv.dipendenza_anziani_pct DESC) ELSE NULL END AS rank_procapite_eu,
    -- % distance from the EU27 country average
    ROUND(
        (cv.dipendenza_anziani_pct - AVG(cv.dipendenza_anziani_pct) FILTER (WHERE cv.is_eu) OVER (PARTITION BY cv.year))
        / NULLIF(ABS(AVG(cv.dipendenza_anziani_pct) FILTER (WHERE cv.is_eu) OVER (PARTITION BY cv.year)), 0) * 100, 1
