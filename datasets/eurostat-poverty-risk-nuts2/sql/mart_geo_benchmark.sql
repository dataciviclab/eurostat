-- mart_geo_benchmark — At-risk-of-poverty rate by NUTS2 region: EU-wide
-- benchmark analytics.
--
...
SELECT
    ...
    CASE
        WHEN b.unit = 'PC' THEN ROUND(AVG(b.value) OVER (PARTITION BY b.year, b.nuts_level, b.unit), 2)
        ELSE NULL
    ...
    -- Percentile within the EU (same year, nuts_level, unit)
    CASE
        WHEN b.unit = 'PC' THEN ROUND(PERCENT_RANK() OVER (PARTITION BY b.year, b.nuts_level, b.unit ORDER BY b.value), 4)
        ELSE NULL
