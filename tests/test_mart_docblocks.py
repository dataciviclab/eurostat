...
# dataset slugs that legitimately own them. A docblock title may only use
# the keyword of ITS dataset (or a generic one like 'by country'/'region').
# A keyword can have MULTIPLE owners when the subject is shared across
# dataflows (e.g. 'population': DEMO_R_D2JAN and DEMO_R_PJANGRP3).
SUBJECT_OWNER: dict[str, set[str]] = {
    "area": {"eurostat-area-nuts3"},
    "business demography": {
        "eurostat-business-demography-nuts3",
        "eurostat-bd-hgnace2-r3-nuts3",
    },
    "enterprise births": {"eurostat-bd-hgnace2-r3-nuts3"},
    ...
    "population": {
        "eurostat-demo-r-pjangrp3-nuts3",
        "eurostat-pop-nuts3",
    },
...
def test_docblock_title_matches_owner(mart):
    ...
    for slug_dir in sorted((REPO / "datasets").iterdir()):
        ...
        owners = SUBJECT_OWNER[matched[0]]
        own_slug = slug_dir.name
        assert own_slug in owners, (
            f"mart_{mart} of {own_slug} has title referencing "
            f"another dataset's subject: '{title}' (owners: {owners})"
        )
